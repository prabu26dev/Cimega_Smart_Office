...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if(!userData.roles.includes('bendahara') && !userData.roles.includes('kepsek') && !userData.roles.includes('ops')){
    alert('Akses Ditolak: Anda tidak memiliki otoritas Keuangan (Bendahara).');
    window.location.href = '../dashboard/dashboard.html';
    return;
  }

  // Set Profile info in UI
  document.getElementById('financeNama').textContent = userData.nama;
  document.getElementById('financeSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function renderSidebar() {
  const sidebar = document.getElementById('financeMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.BendaharaModules || {}).forEach(k => {
    const mod = window.BendaharaModules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.BendaharaModules[modId], item };
...
function renderDocList({ mod, item }) {
  ...
  const catKey = 'bendahara_' + item.id;
  const docs = _kontenCache[catKey] || [];
  
  // Custom glowing button warna hijau (#00ff88) untuk keuangan
  let html = `
    <div class="card" style="cursor:pointer;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-direction:column;min-height:160px;transition:all 0.3s" onclick="openNewDoc()">
      ...
      <div style="font-weight:600;color:#00ff88">Catat Berkas Baru</div>
    </div>
  `;
...
function openDoc(docId) {
  const catKey = 'bendahara_' + currentItem.item.id;
  activeDocId = docId;
...
function renderForm(item, savedData = {}) {
  ...
  item.components.forEach(c => {
    ...
    div.innerHTML = `<label class="form-label">${c.label}</label>
                     <textarea class="form-control" id="form_${c.id}" placeholder="Ketik input acuan raw (mentah) dari ${c.label}...">${savedData[c.id] || ''}</textarea>`;
...
async function saveDoc() {
  ...
  if(!title) { alert('Kolom Judul Arsip Keuangan tidak boleh kosong!'); return; }
  
  const data = getFormData();
  const catKey = 'bendahara_' + currentItem.item.id;
  const docId = activeDocId || 'bendahara_' + Date.now();
  ...
  try {
    ...
    activeDocId = docId;
    alert('Log Arsip Finansial berhasil disimpan di sistem cloud.');
  } catch(e) {
    console.error(e);
