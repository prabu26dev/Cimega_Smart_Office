...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if(!userData.roles.includes('guru') && !userData.roles.includes('kepsek')){
    alert('Akses Ditolak: Anda tidak memiliki otoritas Guru/Pembina Ekskul.');
    window.location.href = '../dashboard/dashboard.html';
    return;
  }

  // Set Profile info in UI
  document.getElementById('ekskulNama').textContent = userData.nama;
  document.getElementById('ekskulSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function renderSidebar() {
  const sidebar = document.getElementById('ekskulMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.EkskulModules || {}).forEach(k => {
    const mod = window.EkskulModules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.EkskulModules[modId], item };
...
function renderDocList({ mod, item }) {
  ...
  const catKey = 'ekskul_' + item.id;
  const docs = _kontenCache[catKey] || [];
  
  // Custom glowing button warna merah crimson (#ff4757)
  let html = `
    <div class="card" style="cursor:pointer;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-direction:column;min-height:160px;transition:all 0.3s" onclick="openNewDoc()">
      ...
      <div style="font-weight:600;color:var(--red)">Catat Agenda / Log Latihan</div>
    </div>
  `;
...
function openNewDoc() {
  ...
  document.getElementById('editorTitle').placeholder = 'Arsip Ekskul ' + currentItem.item.nama;
  renderForm(currentItem.item);
  showPage('editor');
}

function openDoc(docId) {
  const catKey = 'ekskul_' + currentItem.item.id;
  activeDocId = docId;
...
function renderForm(item, savedData = {}) {
  ...
  item.components.forEach(c => {
    ...
    div.innerHTML = `<label class="form-label">${c.label}</label>
                     <textarea class="form-control" id="form_${c.id}" placeholder="Ketik input/catatan mengenai ${c.label}...">${savedData[c.id] || ''}</textarea>`;
...
async function saveDoc() {
  ...
  if(!title) { alert('Kolom Judul Administrasi Pembina Ekskul tidak boleh kosong!'); return; }
  
  const data = getFormData();
  const catKey = 'ekskul_' + currentItem.item.id;
  const docId = activeDocId || 'ekskul_' + Date.now();
  ...
  try {
    ...
    activeDocId = docId;
    alert('Log Administrasi / Bakat Ekstrakurikuler berhasil ditanam di Cloud!');
  } catch(e) {
    console.error(e);
    alert('Koneksi firebase terputus saat penyimpanan!');
  }
}
...
async function generateAI() {
  ...
  Object.keys(data).forEach(k => {
    promptText = promptText.replace(new RegExp(`{{${k}}}`, 'g'), data[k] || '(Data belum dilengkapi)');
  });
  
  if (typeof window.sendPromptToAI === 'function') {
      window.sendPromptToAI(promptText);
  } else {
      alert("AI Prompt (Tempelkan pada AI Pelatih Ekskul Cimega):\n\n" + promptText);
  }
}
