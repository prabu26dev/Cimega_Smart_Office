...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if(!userData.roles.includes('guru') && !userData.roles.includes('kepsek') && !userData.roles.includes('ops')){
    alert('Akses Ditolak: Anda tidak terdaftar sebagai staf Fasilitator Kelas.');
    window.location.href = '../dashboard/dashboard.html';
    return;
  }

  // Set Profile info in UI
  document.getElementById('fasilNama').textContent = userData.nama;
  document.getElementById('fasilSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function renderSidebar() {
  const sidebar = document.getElementById('fasilMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.FasilitatorP5Modules || {}).forEach(k => {
    const mod = window.FasilitatorP5Modules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.FasilitatorP5Modules[modId], item };
...
function renderDocList({ mod, item }) {
  ...
  const catKey = 'fasilp5_' + item.id;
  const docs = _kontenCache[catKey] || [];
  
  // Custom glowing button warna merah muda/rose (var(--rose))
  let html = `
    <div class="card" style="cursor:pointer;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-direction:column;min-height:160px;transition:all 0.3s" onclick="openNewDoc()">
      ...
      <div style="font-weight:600;color:var(--rose)">Buat Baru</div>
    </div>
  `;
...
function openNewDoc() {
  ...
  document.getElementById('editorTitle').placeholder = 'Judul Lembar ' + currentItem.item.nama;
  renderForm(currentItem.item);
  showPage('editor');
}

function openDoc(docId) {
  const catKey = 'fasilp5_' + currentItem.item.id;
  activeDocId = docId;
...
function renderForm(item, savedData = {}) {
  ...
  item.components.forEach(c => {
    ...
    div.innerHTML = `<label class="form-label">${c.label}</label>
                     <textarea class="form-control" id="form_${c.id}" placeholder="Ketik draf kasar/raw info ${c.label}...">${savedData[c.id] || ''}</textarea>`;
...
async function saveDoc() {
  ...
  if(!title) { alert('Kolom Judul Lembar tidak boleh kosong!'); return; }
  
  const data = getFormData();
  const catKey = 'fasilp5_' + currentItem.item.id;
  const docId = activeDocId || 'fasilp5_' + Date.now();
  ...
  try {
    ...
    activeDocId = docId;
    alert('Log Jurnal Fasilitator Kelompok berhasil direkam.');
  } catch(e) {
    console.error(e);
    alert('Sinkronisasi Firebase gagal!');
  }
}
...
async function generateAI() {
  ...
  if (typeof window.sendPromptToAI === 'function') {
      ...
      alert("AI Prompt (Salin instruksi pakar ini ke Chat Cimega AI):\n\n" + promptText);
  }
}
