...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if(!userData.roles.includes('guru') && !userData.roles.includes('kepsek')){
    alert('Akses Ditolak: Anda tidak terdaftar sebagai staf Pendidik/Guru.');
    window.location.href = '../dashboard/dashboard.html';
    return;
  }

  // Set Profile info in UI
  document.getElementById('gpkNama').textContent = userData.nama;
  document.getElementById('gpkSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function renderSidebar() {
  const sidebar = document.getElementById('gpkMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.GpkModules || {}).forEach(k => {
    const mod = window.GpkModules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.GpkModules[modId], item };
...
function renderDocList({ mod, item }) {
  ...
  const catKey = 'gpk_' + item.id;
  const docs = _kontenCache[catKey] || [];
  
  // Custom glowing button warna ungu (#9b59b6) untuk Inklusi
  let html = `
    <div class="card" style="cursor:pointer;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-direction:column;min-height:160px;transition:all 0.3s" onclick="openNewDoc()">
      ...
      <div style="font-weight:600;color:#a855f7">Rekam Asesmen Baru</div>
    </div>
  `;
...
function openNewDoc() {
  ...
  document.getElementById('editorTitle').placeholder = 'Log Jurnal ' + currentItem.item.nama;
  renderForm(currentItem.item);
  showPage('editor');
}

function openDoc(docId) {
  const catKey = 'gpk_' + currentItem.item.id;
  activeDocId = docId;
...
function renderForm(item, savedData = {}) {
  ...
  item.components.forEach(c => {
    ...
    div.innerHTML = `<label class="form-label">${c.label}</label>
                     <textarea class="form-control" id="form_${c.id}" placeholder="Ketik draf catatan ABK dari ${c.label}...">${savedData[c.id] || ''}</textarea>`;
...
async function saveDoc() {
  ...
  if(!title) { alert('Kolom Judul Asesmen Inklusi tidak boleh kosong!'); return; }
  
  const data = getFormData();
  const catKey = 'gpk_' + currentItem.item.id;
  const docId = activeDocId || 'gpk_' + Date.now();
  ...
  try {
    ...
    activeDocId = docId;
    alert('Log Terapi / Pendampingan Siswa Inklusi (ABK) dilindungi Penuh!');
  } catch(e) {
    console.error(e);
    alert('Terdapat masalah saat menyinkronkan data medid.');
  }
}
...
async function generateAI() {
  ...
  if (typeof window.sendPromptToAI === 'function') {
      ...
      alert("AI Prompt (Salin instruksi Orthopedagog ini ke Chat AI Cimega):\n\n" + promptText);
  }
}
