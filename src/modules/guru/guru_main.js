...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('guruNama').textContent = userData.nama;
  document.getElementById('guruSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function listenKonten() {
  ...
  onSnapshot(q, (snap) => {
    ...
    snap.forEach(docSnap => {
      ...
      // Pastikan guru hanya melihat miliknya sendiri (owner == userData.id) kecuali dibagi khusus
      // Namun untuk mempermudah, kita filter client-side saja
    ...
    // Jika sedang buka halaman modul, refresh list
    if (currentItem) {
...
function renderSidebar() {
  const sidebar = document.getElementById('guruMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.GuruModules || {}).forEach(k => {
    const mod = window.GuruModules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.GuruModules[modId], item };
...
function renderDocList({ mod, item }) {
  ...
  let html = `
    <div class="card" style="cursor:pointer;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-direction:column;min-height:160px;transition:all 0.3s" onclick="openNewDoc()">
      ...
      <div style="font-weight:600;color:var(--cyan)">Buat Baru</div>
    </div>
  `;
...
function openNewDoc() {
  ...
  document.getElementById('editorTitle').placeholder = 'Nama Dokumen ' + currentItem.item.nama;
  renderForm(currentItem.item);
  showPage('editor');
}

function openDoc(docId) {
  const catKey = 'guru_' + currentItem.item.id;
  activeDocId = docId;
...
function renderForm(item, savedData = {}) {
  ...
  item.components.forEach(c => {
    ...
    div.innerHTML = `<label class="form-label">${c.label}</label>
                     <textarea class="form-control" id="form_${c.id}" placeholder="Ketik data untuk ${c.label}...">${savedData[c.id] || ''}</textarea>`;
...
async function saveDoc() {
  ...
  if(!title) { alert('Harap isi nama dokumen!'); return; }
  
  const data = getFormData();
  const catKey = 'guru_' + currentItem.item.id;
  const docId = activeDocId || 'guru_' + Date.now();
  ...
  try {
    ...
    activeDocId = docId;
    alert('Dokumen Guru berhasil disimpan secara aman di cloud!');
  } catch(e) {
    console.error(e);
...
async function generateAI() {
  ...
  Object.keys(data).forEach(k => {
    promptText = promptText.replace(new RegExp(`{{${k}}}`, 'g'), data[k] || '(Kosong)');
  });
  
  // Membuka jembatan AI jika ada
  if (typeof window.sendPromptToAI === 'function') {
      window.sendPromptToAI(promptText);
  } else {
      alert("AI Prompt (Salin dan masukkan ke Chat Cimega AI):\n\n" + promptText);
  }
}
