...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Guru PAI mengakses halaman ini. Asumsinya dia punya role 'guru' minimal.
  if(!userData.roles.includes('guru') && !userData.roles.includes('kepsek') && !userData.roles.includes('ops')){
    alert('Akses Ditolak: Anda tidak terdaftar sebagai Pendidik.');
    window.location.href = '../dashboard/dashboard.html';
    return;
  }

  // Set Profile info in UI
  document.getElementById('paiNama').textContent = userData.nama;
  document.getElementById('paiSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function renderSidebar() {
  const sidebar = document.getElementById('paiMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.GuruPaiModules || {}).forEach(k => {
    const mod = window.GuruPaiModules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.GuruPaiModules[modId], item };
...
function renderDocList({ mod, item }) {
  ...
  let html = `
    <div class="card" style="cursor:pointer;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-direction:column;min-height:160px;transition:all 0.3s" onclick="openNewDoc()">
      ...
      <div style="font-weight:600;color:var(--emerald)">Buat Baru</div>
    </div>
  `;
...
function openDoc(docId) {
  const catKey = 'gurupai_' + currentItem.item.id;
  activeDocId = docId;
...
async function saveDoc() {
  ...
  const catKey = 'gurupai_' + currentItem.item.id;
  const docId = activeDocId || 'gurupai_' + Date.now();
  ...
  try {
    ...
    activeDocId = docId;
    alert('Dokumen Guru PAI berhasil disimpan kesiswa DB!');
  } catch(e) {
    console.error(e);
