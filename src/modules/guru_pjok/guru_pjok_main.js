...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('pjokNama').textContent = userData.nama;
  document.getElementById('pjokSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function renderSidebar() {
  const sidebar = document.getElementById('pjokMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.GuruPjokModules || {}).forEach(k => {
    const mod = window.GuruPjokModules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.GuruPjokModules[modId], item };
...
function renderDocList({ mod, item }) {
  ...
  const catKey = 'gurupjok_' + item.id;
  const docs = _kontenCache[catKey] || [];
  
  // Custom orange glow untuk button Buat Baru
  let html = `
    <div class="card" style="cursor:pointer;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-direction:column;min-height:160px;transition:all 0.3s" onclick="openNewDoc()">
      ...
      <div style="font-weight:600;color:var(--orange)">Buat Baru</div>
    </div>
  `;
...
function openDoc(docId) {
  const catKey = 'gurupjok_' + currentItem.item.id;
  activeDocId = docId;
...
async function saveDoc() {
  ...
  const catKey = 'gurupjok_' + currentItem.item.id;
  const docId = activeDocId || 'gurupjok_' + Date.now();
  ...
  try {
    ...
    activeDocId = docId;
    alert('Dokumen Instruktur PJOK berhasil disinkronisasi ke ruang kontrol!');
  } catch(e) {
    console.error(e);
...
async function generateAI() {
  ...
  if (typeof window.sendPromptToAI === 'function') {
      ...
      alert("Format Modul PJOK Ter-generate (Salin ke Chat AI):\n\n" + promptText);
  }
}
