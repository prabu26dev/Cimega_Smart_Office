...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if(!userData.roles.includes('kepsek')){
    alert('Akses Ditolak: Anda bukan Kepala Sekolah.');
    window.location.href = '../dashboard/dashboard.html';
    return;
  }

  // Set Profile info in UI
  document.getElementById('kepNama').textContent = userData.nama;
  document.getElementById('kepSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function renderSidebar() {
  const sidebar = document.getElementById('kepsekMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.KepsekModules || {}).forEach(k => {
    const mod = window.KepsekModules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.KepsekModules[modId], item };
...
function openDoc(docId) {
  const catKey = 'kepsek_' + currentItem.item.id;
  activeDocId = docId;
...
async function saveDoc() {
  ...
  const catKey = 'kepsek_' + currentItem.item.id;
  const docId = activeDocId || 'kep_' + Date.now();
...
async function generateAI() {
  ...
  // Here we would typically call window.openSideChat(promptText) or forward it to an AI engine
  // Since we are leveraging shared ai_helper.js:
  alert("AI Prompt yang akan digenerate:\n\n" + promptText + "\n\n(Catatan: Buka menu Chat AI untuk memproses prompt ini atau integrasikan dengan Firebase Function sesuai arsitektur Cimega).");
}
