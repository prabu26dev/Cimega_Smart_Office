...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Karena ini Koordinator, hak akses bisa dari guru atau kepsek
  if(!userData.roles.includes('guru') && !userData.roles.includes('kepsek') && !userData.roles.includes('ops')){
    alert('Akses Ditolak: Anda tidak terdaftar sebagai staf kependidikan.');
    window.location.href = '../dashboard/dashboard.html';
    return;
  }

  // Set Profile info in UI
  document.getElementById('kokuNama').textContent = userData.nama;
  document.getElementById('kokuSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function renderSidebar() {
  const sidebar = document.getElementById('kokuMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.KordKokuModules || {}).forEach(k => {
    const mod = window.KordKokuModules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.KordKokuModules[modId], item };
...
function renderDocList({ mod, item }) {
  ...
  const catKey = 'kordkoku_' + item.id;
  const docs = _kontenCache[catKey] || [];
  
  // Custom glowing button (menggunakan warna magenta)
  let html = `
    <div class="card" style="cursor:pointer;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-direction:column;min-height:160px;transition:all 0.3s" onclick="openNewDoc()">
      ...
      <div style="font-weight:600;color:#ff00ff">Buat Baru</div>
    </div>
  `;
...
function openNewDoc() {
  ...
  document.getElementById('editorTitle').placeholder = 'Judul Dokumen ' + currentItem.item.nama;
  renderForm(currentItem.item);
  showPage('editor');
}

function openDoc(docId) {
  const catKey = 'kordkoku_' + currentItem.item.id;
  activeDocId = docId;
...
function renderForm(item, savedData = {}) {
  ...
  item.components.forEach(c => {
    ...
    div.innerHTML = `<label class="form-label">${c.label}</label>
                     <textarea class="form-control" id="form_${c.id}" placeholder="Silakan input acuan raw (mentah) dari ${c.label}...">${savedData[c.id] || ''}</textarea>`;
...
async function saveDoc() {
  ...
  if(!title) { alert('Harap berikan nama arsip dokumen!'); return; }
  
  const data = getFormData();
  const catKey = 'kordkoku_' + currentItem.item.id;
  const docId = activeDocId || 'kordkoku_' + Date.now();
  ...
  try {
    ...
    activeDocId = docId;
    alert('Log Administratif Proyek (P5) berhasil disimpan!');
  } catch(e) {
    console.error(e);
    alert('Koneksi terputus! Gagal menanam dokumen di server.');
  }
}
...
async function generateAI() {
  ...
  Object.keys(data).forEach(k => {
    promptText = promptText.replace(new RegExp(`{{${k}}}`, 'g'), data[k] || '(Data belum diisi)');
  });
  
  if (typeof window.sendPromptToAI === 'function') {
      window.sendPromptToAI(promptText);
  } else {
      alert("AI Prompt (Salin instruksi pakar ini ke Cimega AI):\n\n" + promptText);
  }
}
