...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Pustakawan biasanya dipegang oleh staf TU atau Guru Khusus.
  if(!userData.roles.includes('tu') && !userData.roles.includes('guru') && !userData.roles.includes('kepsek')){
    alert('Akses Ditolak: Anda tidak memiliki otoritas Administrator Perpustakaan.');
    window.location.href = '../dashboard/dashboard.html';
    return;
  }

  // Set Profile info in UI
  document.getElementById('pustakawanNama').textContent = userData.nama;
  document.getElementById('pustakawanSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function renderSidebar() {
  const sidebar = document.getElementById('pustakawanMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.PustakawanModules || {}).forEach(k => {
    const mod = window.PustakawanModules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.PustakawanModules[modId], item };
...
function renderDocList({ mod, item }) {
  ...
  const catKey = 'pus_' + item.id;
  const docs = _kontenCache[catKey] || [];
  
  // Custom glowing button warna Turquoise/Teal (#00cec9)
  let html = `
    <div class="card" style="cursor:pointer;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-direction:column;min-height:160px;transition:all 0.3s" onclick="openNewDoc()">
      ...
      <div style="font-weight:600;color:var(--teal)">Input Data Pustaka / Rekam Sirkulasi</div>
    </div>
  `;
...
function openNewDoc() {
  ...
  document.getElementById('editorTitle').placeholder = 'Arsip Pustaka ' + currentItem.item.nama;
  renderForm(currentItem.item);
  showPage('editor');
}

function openDoc(docId) {
  const catKey = 'pus_' + currentItem.item.id;
  activeDocId = docId;
...
function renderForm(item, savedData = {}) {
  ...
  item.components.forEach(c => {
    ...
    div.innerHTML = `<label class="form-label">${c.label}</label>
                     <textarea class="form-control" id="form_${c.id}" placeholder="Ketik draf / input catatan terkait ${c.label}...">${savedData[c.id] || ''}</textarea>`;
...
async function saveDoc() {
  ...
  if(!title) { alert('Kolom Judul Arsip Pustaka tidak boleh kosong!'); return; }
  
  const data = getFormData();
  const catKey = 'pus_' + currentItem.item.id;
  const docId = activeDocId || 'pus_' + Date.now();
  ...
  try {
    ...
    activeDocId = docId;
    alert('Log Arsip Perpustakaan (Katalog / Sirkulasi) di-backup ke server!');
  } catch(e) {
    console.error(e);
    alert('Gagal menyinkronkan data perpustakaan.');
  }
}
...
async function generateAI() {
  ...
  Object.keys(data).forEach(k => {
    promptText = promptText.replace(new RegExp(`{{${k}}}`, 'g'), data[k] || '(Data kosong)');
  });
  
  if (typeof window.sendPromptToAI === 'function') {
      window.sendPromptToAI(promptText);
  } else {
      alert("AI Prompt (Salin instruksi Kepala Perpustakaan ini ke Agen AI Anda):\n\n" + promptText);
  }
}
