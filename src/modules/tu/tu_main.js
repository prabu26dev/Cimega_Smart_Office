...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if(!userData.roles.includes('tu') && !userData.roles.includes('ops') && !userData.roles.includes('kepsek')){
    alert('Akses Ditolak: Anda tidak memiliki otoritas Tata Usaha (TU).');
    window.location.href = '../dashboard/dashboard.html';
    return;
  }

  // Set Profile info in UI
  document.getElementById('tuNama').textContent = userData.nama;
  document.getElementById('tuSekolah').textContent = userData.sekolah || 'Tidak ada instansi';
...
function renderSidebar() {
  const sidebar = document.getElementById('tuMenu');
  if(!sidebar) return;
  sidebar.innerHTML = '';
  
  Object.keys(window.TuModules || {}).forEach(k => {
    const mod = window.TuModules[k];
...
function openCategory(modId, item) {
  currentItem = { mod: window.TuModules[modId], item };
...
function renderDocList({ mod, item }) {
  ...
  const catKey = 'tu_' + item.id;
  const docs = _kontenCache[catKey] || [];
  
  // Custom glowing button warna oranye (#ff9f43)
  let html = `
    <div class="card" style="cursor:pointer;border:2px dashed var(--border);display:flex;align-items:center;justify-content:center;flex-direction:column;min-height:160px;transition:all 0.3s" onclick="openNewDoc()">
      ...
      <div style="font-weight:600;color:var(--orange)">Catat Register Baru</div>
    </div>
  `;
...
function openDoc(docId) {
  const catKey = 'tu_' + currentItem.item.id;
  activeDocId = docId;
...
function renderForm(item, savedData = {}) {
  ...
  item.components.forEach(c => {
    ...
    div.innerHTML = `<label class="form-label">${c.label}</label>
                     <textarea class="form-control" id="form_${c.id}" placeholder="Ketik input draf raw dari manual book ${c.label}...">${savedData[c.id] || ''}</textarea>`;
...
async function saveDoc() {
  ...
  if(!title) { alert('Kolom Judul Arsip Buku TU tidak boleh kosong!'); return; }
  
  const data = getFormData();
  const catKey = 'tu_' + currentItem.item.id;
  const docId = activeDocId || 'tu_' + Date.now();
  ...
  try {
    ...
    activeDocId = docId;
    alert('Log Arsip Fisik berhasil dilindungi dalam server awan (Cloud)!');
  } catch(e) {
    console.error(e);
...
async function generateAI() {
  ...
  if (typeof window.sendPromptToAI === 'function') {
      ...
      alert("AI Prompt (Tempelkan pada AI Cimega Chatbot Anda):\n\n" + promptText);
  }
}
