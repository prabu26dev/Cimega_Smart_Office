window.CimegaChat = {
  ...
  injectUI: function(containerId) {
    ...
    const sidebarTitle = this.currentTab === 'kepsek' ? 'KOLEGA KEPALA SEKOLAH' : 'PERSONIL SEKOLAH';
  ...
  handleFileUpload: async function(e) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        // VALIDASI
        if (!isKepsek && !file.type.startsWith('image/')) {
          alert(`Hanya Kepala Sekolah yang dapat mengirim dokumen. Guru/Staff hanya diizinkan mengirim gambar.`);
          continue;
        }
